...
**触发：** 任何一次状态变更后，**同一轮内**执行（不拖延，CLAUDE.md 既有规则）。状态变更包括：决策被 Founder 确认 / 产出新方案文档 / 完成任务 / 阶段或方向变化 / 失败计数变化。**对话中任何 Claude 提出的建议/方案** 也触发 §1c 更新（不需等 Founder 确认）。

---

## 同步顺序（固定）

0. **【新增·防漏】Claude 对话级建议 → §1c（立即）：** 凡本轮对话中 Claude 提出的方案/建议/行动项，无论是否被 Founder 确认，**当场写入 CURRENT_STATE §1c 对话级建议暂存区**，标注状态（待确认/可执行/待查）。禁止只留在对话中不落文件。
1. **CURRENT_STATE.md**（运行权威）：更新"下一步" + §1b + §1c + 版本号 + 顶部最后更新摘要。
