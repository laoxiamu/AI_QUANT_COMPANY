...
| P0-RES-018 | A-4重定义普查："首个永续上市打开做空通道"（不看收益，R2-B3） | ✅**完成（2026-07-21）**，Claude验收裁决=**不够格立项，本线归档** | Codex执行✅；Claude验收✅ | R2生成性审计B3 | 有效样本 `existing_spot_then_perp & 数据完整`=**89 < 100门槛**；且**已退市率37.1%**（vs 同步上市5.5%/零基数14.5%）=幸存者偏差风险最高的一组，2022年13个样本仅3个数据完整。**裁决理由：出口判据是我自己预先冻结的，89<100 即归档，不因"差一点"放宽——这正是防局部修补的纪律。** 事件表686行留档`06_RESEARCH/DATA/LISTING_EVENTS/`供未来复用；若将来PIT数据源改善使有效样本过100可重提 | `REPORT_P0RES018_A4_LISTING_CENSUS_20260721.md` |
| P0-RES-017 | 扫描器周期语义修复 + EVENT_LEDGER_V1 落地（R2-A-02 + R2-B1） | ✅**完成（2026-07-21）**，Claude验收=PASS | Codex执行✅；Claude验收✅ | R2生成性审计 | **①周期缺陷已修**：新增`funding_per_settlement`/`interval_hours`（实测推断）/`funding_per_day`，排序与阈值统一改用日化值（8h档每期-0.3%≡日-0.9%）；实测样例暴露问题严重性——**TUSDT 每期-0.537% 实为1h档=日化-12.88%**（旧口径当8h档处理，低估8倍）；OI从"只补前8名"改为全候选补齐。②**EVENT_LEDGER_V1已运行**：SQLite+parquet快照、写入器/回填器/独立结算器三件套，near_miss自动判定、lookahead机器校验、4个历史快照已回填（标backfilled不参与技能评分）。③单测验证1h:8h日化比=8.0 | `REPORT_P0RES017_SCANNER_LEDGER_20260721.md`；`06_RESEARCH/INVESTMENT_RESEARCH/EVENT_LEDGER_V1_SCHEMA.md` |
| P0-GOV-013 | **独立第三方全方位审计（Codex，Founder 2026-07-20直接指示）**：顶层方案/方向/方法/架构→底层任务计划与文档状态全覆盖 | 🟢已派（2026-07-20，运行中） | Codex独立审计人；Claude接收（不预设采纳） | Founder指示 | 任务书刻意零主观意见/零引导/零观点约束（只限定只读+不碰Holdout+不花钱等动作边界），一切现行决策/制度/Claude自己的审计报告均在可质疑范围；log=`ext_audit_20260720_run.log`；出报告后Claude逐条回应（同意/异议均给理由），D级项上呈Founder | `04_AI_TEAM/CODEX_TASKS/TASK_EXTERNAL_AUDIT_CODEX_20260720.md`；待产出`STAGE_AUDITS/EXTERNAL_DEEP_AUDIT_CODEX_2026-07-20.md` |
