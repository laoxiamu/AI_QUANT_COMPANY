#!/usr/bin/env python3
"""Phase 1 P1-04: regime-first long-only 90-day TSMOM."""
...
def desired_long_position(trend_signal: int, trend_regime: bool) -> int:
    return int(trend_signal == 1 and trend_regime)
...
def load_pre_holdout() -> tuple[
    ...
    for symbol, config in SYMBOL_CONFIG.items():
        ...
        bars["desired_long"] = [
            desired_long_position(signal, regime)
...
def filtered_period_diagnostic(
    ...
    return {
        "definition": (
            "long TSMOM candidate bars excluded by ADX; next-open 4H "
            "returns, equal-weight diagnostic without costs"
        ),
        "by_symbol": by_symbol,
...
def report_payload(
    ...
    payload = {
        ...
        "filtered_periods": filtered_period_diagnostic(bars),
    ...
    payload["regime_filter_effect"] = {
        "net_profit_difference_vs_unfiltered_long": float(
            main["net_profit"]
            - payload["unfiltered_long_only"]["net_profit"]
        ),
        "sharpe_difference_vs_unfiltered_long": float(
            main["sharpe"]
            - payload["unfiltered_long_only"]["sharpe"]
        ),
        "additional_segments_from_regime_reentry": int(
            main["trade_count"]
            - payload["unfiltered_long_only"]["trade_count"]
        ),
    }
    return net, payload
...
def plot_equity(equity: pd.Series) -> None:
    ...
    axis.set_title("P1-04 Regime-First Long TSMOM")
    axis.set_xlabel("UTC time")
    axis.set_ylabel("Equity (USDT)")
    axis.grid(alpha=0.2)
    figure.tight_layout()
    figure.savefig(OUTPUT / "p1_04_tsmom_regime_long_equity_curve.png", dpi=160)
    plt.close(figure)
...
def main() -> None:
    ...
    output = {
        "experiment": "v6_tsmom_regime_long_v3",
        "holdout_accessed": False,
        "parameters": {
            "lookback_bars": LOOKBACK_BARS,
            "lookback_days": 90,
            "signal": "long when close[t] > close[t-540] and trend regime",
    ...
    with (OUTPUT / "p1_04_tsmom_regime_long_metrics.json").open("w") as handle:
