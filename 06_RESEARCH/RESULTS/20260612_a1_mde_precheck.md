...
## MDE 双参数对照（B4 补充，2026-06-13）

公式：

```text
MDE_alpha_only = z_(0.95) * σ / sqrt(n)
MDE_alpha_beta = (z_(0.95) + z_(0.80)) * σ / sqrt(n) = 2.4869 * σ / sqrt(n)
n = 163
```

| horizon | σ | n | MDE_alpha_only | MDE_alpha_beta | 机制区间下沿 1.5% | 功效门 alpha_only | 功效门 alpha_beta |
|---|---:|---:|---:|---:|---:|---|---|
| 24h | 0.048529 | 163 | 0.625% | 0.945% | 1.500% | 通过 | 通过 |
| 48h | 0.067676 | 163 | 0.872% | 1.318% | 1.500% | 通过 | 通过 |
| 72h | 0.083279 | 163 | 1.073% | 1.622% | 1.500% | 通过 | 不通过 |

说明：若正式预登记采用 alpha+beta（α=0.05 单侧、power=0.8）作为功效门，72h horizon 单独不满足 1.5% 下沿；24h/48h 仍满足。该补充仅是功效口径对照，不计算事件后实际收益。
