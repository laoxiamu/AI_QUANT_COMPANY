...
## 预先澄清（不要再问，按此实现）
- **buffer_breach vs liquidation 两级事件，都记、区分**：保证金跌破"缓冲线"（缓冲=维持保证金×3，预登记口径）但**未**跌破交易所最低维持保证金 → 记 `buffer_breach`（预警/触发补保证金或减仓，不强平）；**仅当**跌破交易所最低维持保证金 → 记 `liquidation`（短腿被强平、delta 暴露）。两者都进事件日志，分别计数。
- 一切设计歧义一律取**保守默认 + 在自测报告里写明所采默认**，**不得停下来问、不得用浏览器/computer-use**。
